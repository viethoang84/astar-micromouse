...
Window::Window(QWidget *parent)
    : QMainWindow(parent),
      ...
      m_mazeSizeSpinBox(new QSpinBox()),
      m_generateMazeButton(new QPushButton("Generate Maze")),
  ...
  // Add maze size spinbox (n = 1..100) and Generate button
  QLabel *sizeLabel = new QLabel("n =");
  m_mazeSizeSpinBox->setRange(1, 100);
  m_mazeSizeSpinBox->setValue(16);
  configLayout->addWidget(sizeLabel, 2, 0, 1, 1);
  configLayout->addWidget(m_mazeSizeSpinBox, 2, 1, 1, 1);
  configLayout->addWidget(m_generateMazeButton, 2, 2, 1, 3);
  connect(m_generateMazeButton, &QPushButton::clicked, this,
          &Window::onGenerateMazeButtonPressed);
...
void Window::onGenerateMazeButtonPressed() {
  int n = m_mazeSizeSpinBox->value();
  Maze *maze = Maze::fromGenerated(n);
  if (maze != nullptr) {
    updateMaze(maze);
    m_currentMazeFile = QString();
    m_mazeFileComboBox->setCurrentIndex(-1);
    stats->resetAll();
  }
}
